...
function parseGlobal(gl) {
    ...
    return {
        ...
        globalHost: host,
        globalPort: Number(port)
    }
}

const globalVariable = process.env.DB_CONNECTION
const { globalUser, globalPassword, globalHost, globalPort } = parseGlobal(globalVariable)


const client = new Client({ user: globalUser, password: globalPassword, host: globalHost, port: globalPort })
